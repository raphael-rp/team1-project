...
// CURRENT_USER is set inline by views/gamification.ejs from the logged-in
// session account (there is no client-side "auth.js" — accounts live in MySQL).
...
function getGamificationData() {
    ...
    if (!allData[CURRENT_USER]) {

        // Initialize a brand new user profile
        allData[CURRENT_USER] = {
            xp: 0,
            level: 1,
...
function addXP(amount, actionName) {
    ...
    if (leveledUp) {
        toastMsg += `<br><strong>🎉 You leveled up! Congratulations, you reached Level ${stats.level}!</strong>`;
    }
...
// 5. Live Leaderboard Compiler (current account's real stats + demo peers)
function getLiveLeaderboard() {

    let gamificationDb = JSON.parse(localStorage.getItem("gamification")) || {};
    let stats = gamificationDb[CURRENT_USER] || { xp: 0, level: 1, streak: 0 };

    function totalXPFor(s) {
        let total = 0;
    ...
    let leaderboard = [{
        ...
        email: CURRENT_USER,
        level: stats.level,
        totalXP: totalXPFor(stats),
        isLiveUser: true
    }];

    // Demo peers to make the leaderboard feel alive
    const fallbackPeers = [
